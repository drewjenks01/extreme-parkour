...
class HardwareScandotNN(nn.Module):
    ...
    def forward(self, obs):
        obs[:, self.num_prop+self.num_scan : self.num_prop+self.num_scan+self.num_priv_explicit] = self.estimator(obs[:, :self.num_prop])
        return self.actor(obs, hist_encoding=True, eval=False, scandots_latent=None)
        # return obs, depth_latent
...
def play(args):    
    ...
    # extract int from "phase{int}" using regex
    phase = int(re.search(r'phase(\d+)_', args.exptid).group(1))
    ...
    with torch.no_grad(): 
        ...
        if phase == 1:
            save_file_name = 'traced_model_no_depth.jit'
